...
{
    float w, h;
    //check to see if we are in unit test mode or in game mode. game mode when the if is true
    if(setbackground)
	for(int j = 0; j <amt; j++)
	{
...
{   
    //connects platform with player's width and height
    int width = game->player.width * 15;
    int height = game->player.height * 0.35;
    srand(time(NULL));
    // designs platform and puts them up randomly on width and 1/5 the screen on height
    for(int i = 0; i<amt; i++)
    {
